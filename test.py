...
def problems_test(nrun, draw, s2f=False):
    '''
    一系列函数问题的测试
    :return:
    '''
    ...
    for id in range(len(problems)):
        ...
        igds, hvs = n_run(nrun, problems[id], draw=draw, s2f=s2f)
...
def problem_test(problem, draw=True, s2f=False):
    ...
    if draw:
        draw_scatter3D(model.problem.name(), hv_score, igd, reference_point, x, y, z, model.pareto_front)
...
if __name__ == '__main__':
    ...
    # igds, hvs = n_run(10, problems[], True, s2f=True)
    # res = problems_test(10, False, s2pidf=False)
    # saveRes2Excel("./results/excels/{}.xls".format('moea-d-code'), res)
