...
class MOEADCODE():
    ...
    def diffevolution(self):
        ...
        for i in range(self.pop_size):
            ...
            if random.random()>self.realb:
                type = 2
            child, fit = self.diff_evo_xover2(n, type)
    ...
    def diff_evo_xover2(self, cid, type):
        ...
        for i in range(self.n_var):
            ...
            child2[i] = xi2
            child3[i] = xi3
        fit1 = self.problem.evaluate(child1)
        fit2 = self.problem.evaluate(child2)
        fit3 = self.problem.evaluate(child3)
        ff1 = fitness_function(fit1, self.pop[cid].namda, self.ideal_fitness)
        ff2 = fitness_function(fit2, self.pop[cid].namda, self.ideal_fitness)
        ff3 = fitness_function(fit3, self.pop[cid].namda, self.ideal_fitness)
        if ff1 < ff2 and ff1 < ff3:
            return child1, fit1
        elif ff2 < ff1 and ff2 < ff3:
            return child2, fit2
        else:
            return child3, fit3
